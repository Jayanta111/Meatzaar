...
export default function VendorPaymentMethodsScreen() {
  const theme = useTheme();
  const { isAuthenticated } = useAuthStore();
  const [paymentMethods, setPaymentMethods] = useState<any[]>([]);
  const [loading, setLoading] = useState(true);
  const [showAddModal, setShowAddModal] = useState(false);
  const [newMethod, setNewMethod] = useState({ type: 'UPI', upiId: '', accountNumber: '', ifsc: '', accountName: '' });
  ...
  const addPaymentMethod = async () => {
    ...
    if (newMethod.type === 'BANK' && (!newMethod.accountNumber || !newMethod.ifsc || !newMethod.accountName)) {
    ...
    try {
      const details = newMethod.type === 'UPI' 
        ...
        : { accountNumber: newMethod.accountNumber, ifsc: newMethod.ifsc, accountName: newMethod.accountName, isDefault: paymentMethods.length === 0 };
      ...
      setNewMethod({ type: 'UPI', upiId: '', accountNumber: '', ifsc: '', accountName: '' });
  ...
  const renderPaymentMethod = ({ item }: { item: any }) => (
    <View style={[styles.card, { backgroundColor: theme.cardBackground, borderColor: theme.cardBorder }]}>
      <View style={styles.cardHeader}>
        <View style={[styles.iconContainer, { backgroundColor: item.type === 'UPI' ? '#6739B7' + '20' : '#10B981' + '20' }]}>
          <Ionicons name={item.type === 'UPI' ? 'logo-google' : 'business'} size={24} color={item.type === 'UPI' ? '#6739B7' : '#10B981'} />
        </View>
        <View style={styles.cardInfo}>
          <Text style={[styles.cardType, { color: theme.text }]}>{item.type === 'BANK' ? 'Bank Account' : item.type}</Text>
          <Text style={[styles.cardDetails, { color: theme.textSecondary }]}>
            {item.type === 'UPI' ? item.details.upiId : `${item.details.accountName} - ${item.details.accountNumber.slice(-4)}`}
          </Text>
        </View>
        {item.isDefault && (
        ...
        <View style={[styles.modalContainer, { backgroundColor: theme.background }]}>
          ...
          <View style={styles.modalContent}>
            <View style={styles.typeSelector}>
              ...
              <TouchableOpacity
                style={[styles.typeOption, { backgroundColor: newMethod.type === 'BANK' ? theme.primary + '20' : theme.cardBackground, borderColor: newMethod.type === 'BANK' ? theme.primary : theme.border }]}
                onPress={() => setNewMethod({ ...newMethod, type: 'BANK' })}
              >
                <Ionicons name="business" size={28} color={newMethod.type === 'BANK' ? '#10B981' : theme.textSecondary} />
                <Text style={[styles.typeText, { color: newMethod.type === 'BANK' ? theme.primary : theme.text }]}>Bank</Text>
              </TouchableOpacity>
            </View>
            ...
            {newMethod.type === 'UPI' ? (
              ...
              <>
                <View style={[styles.inputContainer, { backgroundColor: theme.cardBackground, borderColor: theme.border }]}>
                  <Text style={[styles.inputLabel, { color: theme.textSecondary }]}>Account Number</Text>
                  <TextInput
                    style={[styles.input, { color: theme.text }]}
                ...
                <View style={[styles.inputContainer, { backgroundColor: theme.cardBackground, borderColor: theme.border }]}>
                  <Text style={[styles.inputLabel, { color: theme.textSecondary }]}>IFSC Code</Text>
                  <TextInput
                    style={[styles.input, { color: theme.text }]}
                    placeholder="SBIN0001234"
                    placeholderTextColor={theme.textMuted}
                    value={newMethod.ifsc}
                    onChangeText={(text) => setNewMethod({ ...newMethod, ifsc: text.toUpperCase() })}
                ...
                <View style={[styles.inputContainer, { backgroundColor: theme.cardBackground, borderColor: theme.border }]}>
                  ...
                  <TextInput
                    ...
                    onChangeText={(text) => setNewMethod({ ...newMethod, accountName: text })}
                  />
                </View>
              </>
            )}
